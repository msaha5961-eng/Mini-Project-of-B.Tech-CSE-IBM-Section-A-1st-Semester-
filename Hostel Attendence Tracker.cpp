...
int main() {
    ...
    cout << "Enter attendance (10 students & 7 days):" << endl;
    for (i = 0; i < 10; i++) {
        cout << "Student " << i + 1 << ": ";
        for (j = 0; j < 7; j++) {
            cin >> A[i][j];
    ...
    cout << "\nStudent with Highest Attendance: Student " << topper << " (" << max << " Days)" << endl;

    int daySum[7];
    for (j = 0; j < 7; j++) {
        daySum[j] = 0;
        for (i = 0; i < 10; i++) {
            daySum[j] = daySum[j] + A[i][j];
        }
    }
    ...
    cout << "\nDay with Lowest Overall Attendance: Day " << minDay << endl;
